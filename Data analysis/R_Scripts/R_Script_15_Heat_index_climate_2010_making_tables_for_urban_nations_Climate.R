# 
# ==============================================================================================
# THIS SCRIPT OPENS ALL THE COMBINED HEAT INDEX FILES WITH UHI DATA AND DESCRIPTIVES FOR THE 
# CLIMATE 2010 DATA.  IT SUMMARIZES THE INFORMATION INTO NATIONAL LEVEL UNITS WITH TAGS FOR 
# ALLOWING REGIONAL, SUB-REGIONAL, WORLD BANK INCOME STATUS AND OTHER AGGREGATIONS.  THE DATA ARE 
# STORED AS SUMMARY FILES IN "CLIMATE_FINAL/SUMMARY_TABLES" FOLDERS.  THE SCRIPT CAN BE RUN ONCE 
# FOR BOTH GRUMP AND GLOBCOVER DATA.   
#
...
nation.tables<-function(file_x, landuse, name_file_x){
  ...
  l_file_5<-file_x%>%
    ...
    summarize(model_number = n(),
              ...
              least_dev = mean(Least_Developed),
  ...
  l_file_6<-l_file_5%>%
    ...
    summarize(Nation_Urb_Pop = sum(as.numeric(Pop)),
              region = mean(region),
  ...
  first<-paste("Summary_Climate_", landuse, "_by_Nation", sep = "")
  very_last_parts<-parts_file_name[[1]][6]
  #print(very_last_parts)
  last_parts<-parts_file_name[[1]][5]
  #print(last_parts)
  last <- substr(last_parts, 1, 1)
  #print(last)
  file_name<- paste(first, parts_file_name [[1]][3], parts_file_name[[1]][4], last_parts, very_last_parts, sep="_")
  ...
  l_file_7<-l_file_5%>%
    ...
    summarize(Nation_pop = sum(as.numeric(Pop)),
              region = mean(region),
              sub_region = mean(sub_region),
              dev_status = mean(dev_status),
              urban_extents = n(),
  ...
  # Make names for final file
  parts_file_name<-strsplit(name_file_x, "_")
  first<-paste("Summary_Climate_", landuse, "_by_Nation_w_UHI", sep = "")
  very_last_parts<-parts_file_name[[1]][6]
  last_parts<-parts_file_name[[1]][5]
  last <- substr(last_parts, 1, 1)
  file_name<- paste(first, parts_file_name [[1]][3], parts_file_name[[1]][4], last_parts, very_last_parts, sep="_")
...
path_b<-"E:/July_2017/Pop_&_Temp/GlobCover_HI/Climate_2010/Climate_Final"
...
for(i in 1:length(paths)){
  ...
  final_directory<-file.path(start_directory, "Summary_tables")
