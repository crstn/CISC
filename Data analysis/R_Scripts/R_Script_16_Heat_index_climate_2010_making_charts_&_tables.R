...
# CREATES TWO DATAFRAMES FOR EACH RCP-SSP AND EXAMINES THEM SEPARATELY.  DISTINCTION IS BETWEE 
# LARGE POPULATION URBAN EXTENTS (>2500) AND SMALL POPULATION URBAN EXTENTS (<2500).  THIS 
# DISTINCTION WAS MADE TO LOWER THE NUMBER OF ROWS IN THE CSV FILE FOR EASIER HANDLING.  EACH 
# ANALYSIS IS PERFORMED FOR BOTH SETS OF DATA.  TABLES ARE PRODUCED THAT SUMMARIZE RESULTS.  
#
#
...
path_a<-"E:/July_2017/Pop_&_Temp/GRUMP_HI/Climate_2010/Climate_Final"
path_b<-"E:/July_2017/Pop_&_Temp/GlobCover_HI/Climate_2010/Climate_Final" # MIGHT NEED TO CHANGE THIS!
...
for(j in 1:length(paths)){
  ...
  for(i in 1:length(sum_files)){
    ...
    last_parts<-parts_file_name[[1]][5]
    ssp <- unlist(strsplit(last_parts, split = ".", fixed = TRUE))[1]
    ssp_number<-paste(parts_file_name[[1]][4], ssp, sep = " ")
    ssp_number_for_file<- paste(parts_file_name[[1]][4], ssp, sep="_")
    rcp_number<-parts_file_name[[1]][3]
    heat_wave_number<-parts_file_name[[1]][6]
    ...
    # make only one of each of these 3 charts per RCP
    if(ssp_number == "SSP 1"){
      ...
      #  SUMMARY TABLE OF GLOBAL TEMPERATURES
      ...
      # Create title
      title_q<-paste("Mean global urban heat index for heat wave of ", heat_wave_name, " ", days, ", ", rcp_name, sep = "")
      ...
      # Make file name for chart
      file_name_identifier<-paste(rcp_number, heat_wave_number, sep = "_" )
      file_name_end<-paste(file_name_identifier, ".jpg", sep="")
      ...
      file_name<-paste ("Climate_2010", land_use_name, rcp_number, heat_wave_number, "national_temperature.csv", sep = "_")
      ...
      # Create name for table file
      file_name<-paste ("Climate_2010", land_use_name, rcp_number, heat_wave_number, "sub_regional_temperature.csv", sep = "_")
      full_file_path<-paste(final_HI_table_directory, file_name, sep = "/")
      ...
      # Create name for table file
      file_name<-paste ("Climate_2010", land_use_name, rcp_number, heat_wave_number, "regional_temperature.csv", sep = "_")
      full_file_path<-paste(final_HI_table_directory, file_name, sep = "/")
      ...
      title_q<-paste("Mean global urban Heat Index by GCM for heat wave of ", heat_wave_name, " ", days, ", ", rcp_name, sep = "")
      ...
      ##  Save table with global table but with new dates 
      file_name<-paste ("Climate_2010", land_use_name, rcp_number, heat_wave_number, "global_temperature.csv", sep = "_")
      full_file_path<-paste(final_HI_table_directory, file_name, sep = "/")
...
# FOR PANELING CHARTS WITHOUT FACET
